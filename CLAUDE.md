...
OpenLogi is a Rust workspace of seven application crates — a native,
local-first alternative to Logitech Options+ that controls Logitech mice over
HID++ (via a Logi Bolt receiver, a direct Bluetooth link, or a USB cable).
macOS is the supported platform today; Linux and Windows are stubs that compile
but do nothing. The workspace builds two binaries: `openlogi` (CLI) and
`openlogi-gui` (a GPUI desktop app). An eighth member, `xtask`, is a
`publish = false` build helper (macOS packaging + updater manifest), not part of
the application.
...
Dependency direction: `openlogi-core`, `openlogi-assets`, and `openlogi-hidpp`
are foundation crates with no internal deps; `openlogi-hid` depends on core +
hidpp, `openlogi-hook` on core; `openlogi-cli` and `openlogi-gui` sit on top.
...
| `openlogi-hidpp` | Vendored fork of the `hidpp` crate (from `lus/logy`, upstream v0.3.0); library target stays named `hidpp`. Carries OpenLogi-specific changes (BLE long-report handling). Opts out of workspace lints. |
| `openlogi-hid` | HID++ over `hidpp` (the vendored fork) + `async-hid`: enumerate, DPI (`0x2201`), SmartShift (`0x2111`), control capture (`0x1b04`/`0x2150`). |
...
Rust edition 2024, workspace MSRV `rust-version = 1.85` — but the vendored
`openlogi-hidpp` fork declares `1.87`, so the build's effective MSRV is `1.87`.
The workspace runs Clippy `pedantic` (warn) with `unwrap_used` and `expect_used`
warned; the pre-commit gate runs `cargo clippy --workspace --all-targets -- -D
warnings`, so any of those lints fails the build. (`openlogi-hidpp` opts out of
the workspace lint profile — it is third-party code.)

`unsafe` is denied workspace-wide via `unsafe_code = "deny"`
(`workspace.lints.rust`). Because the level is `deny` (not `forbid`), the
four modules that genuinely need FFI opt back in locally — via an
...
- `openlogi-gui/src/platform/permissions.rs` — IOKit (`IOHIDCheckAccess`) +
  CoreBluetooth privacy-permission FFI for the Settings → Permissions section.
...
- **The `hidpp` feature registry is empty for many of our features.** Resolve a
  ...
  **not** `enumerate_features()` (its `versions: &[]` means a typed
  ...
  `0x1b04`, `0x2150`. See `write::open_feature`. (`hidpp` here is the vendored
  `openlogi-hidpp` fork.)
